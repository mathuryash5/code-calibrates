...
import tqdm
import traceback
from pal.prompt import date_understanding_text_prompt_langchain
import re
...
OUTPUT_PATH = f'eval_results/{args.dataset}_{args.model}_text_k_{args.majority_at}_t_{args.temperature}.jsonl'
os.makedirs(os.path.dirname(OUTPUT_PATH), exist_ok=True)

examples = list(map(json.loads, open(DATA_PATH)))

def extract_answer_date(gen: str):
	last_line = gen.strip()
	REGEX_DATE = r'\d{2}/\d{2}/\d{4}'
	numbers = re.findall(REGEX_DATE, last_line)
	print(numbers)
	return numbers[-1]


itf = interface.TextInterfaceLangChain(
	stop='\n\n\n',
	# get_answer_expr='solution()',
	model=args.model,
	extract_answer=extract_answer_date
...
with open(OUTPUT_PATH, 'a' if args.append else 'w') as f:
	...
	for x in pbar:
		...
		result = copy.copy(x)
		try:
			ans = itf.run(prompt=date_understanding_text_prompt_langchain.DATE_UNDERSTANDING_PROMPT, question=question,
						  majority_at=args.majority_at, temperature=args.temperature,
