...
import tqdm
import traceback
from pal.prompt import object_counting_text_prompt_langchain
...
with open(OUTPUT_PATH, 'a' if args.append else 'w') as f:
	...
	for x in pbar:
		...
		result = copy.copy(x)
		try:
			ans = itf.run(prompt=object_counting_text_prompt_langchain.OBJECT_COUNTING_PROMPT, question=question,
