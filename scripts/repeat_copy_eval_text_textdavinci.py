...
from pal.prompt import repeat_copy_text_prompt_langchain
...
def custom_extract_answer(gen : str):
    answer_prefix = 'The answer is:'
    last_line = gen.strip().split('\n')[-1]
    return last_line[len(answer_prefix):].strip()
...
itf = interface.TextInterfaceLangChain(
    ...
    extract_answer=custom_extract_answer
)
...
with open(OUTPUT_PATH, 'a' if args.append else 'w') as f:
    ...
    for x in pbar:
        ...
        try:
            ans = itf.run(
                prompt=repeat_copy_text_prompt_langchain.REPEAT_COPY_PROMPT,
                question=question,
            ...
            ans = str(ans)
            score = 1 if ans == str(x['target']) else 0
