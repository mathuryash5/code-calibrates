...
from pal.prompt import repeat_copy_prompt_llm_execution
...
OUTPUT_PATH = f'eval_results/{args.dataset}_llm_execution_chatgpt_code_k_{args.majority_at}.jsonl'
...
itf = interface.TextChatInterface(
    ...
    system_message=repeat_copy_prompt_llm_execution.REPEAT_COPY_BETA_SYSTEM_MESSAGE,
...
with open(OUTPUT_PATH, 'a' if args.append else 'w') as f:
    ...
    for x in pbar:
        ...
        try:
            ans = itf.run(
                repeat_copy_prompt_llm_execution.REPEAT_COPY_PROMPT.format(question=question),
