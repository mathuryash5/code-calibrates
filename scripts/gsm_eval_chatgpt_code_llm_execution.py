...
from pal.prompt import math_prompts_code_llm_execution
...
OUTPUT_PATH = f'eval_results/{args.dataset}_chatgpt_llm_execution_k_{args.majority_at}_t_{args.temperature}.jsonl'
os.makedirs(os.path.dirname(OUTPUT_PATH), exist_ok=True)

examples = list(map(json.loads, open(DATA_PATH)))

itf = interface.TextChatCodeInterface(
    stop=None,
    # get_answer_expr='solution()',
    model=args.model,
    verbose=args.verbose,
    system_message=math_prompts_code_llm_execution.MATH_CHAT_BETA_PROMPT,
)
...
with open(OUTPUT_PATH, 'a' if args.append else 'w') as f:
    ...
    for x in pbar:
        ...
        result = copy.copy(x)
        print(math_prompts_code_llm_execution.MATH_CHAT_BETA_PROMPT.format(question=question))
        try:
            ans = itf.run(
                math_prompts_code_llm_execution.MATH_CHAT_BETA_PROMPT.format(question=question),
